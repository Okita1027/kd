...
虽然替代键在数据库中也表现为唯一索引（或唯一约束），但它们在 EF Core 中有不同的语义和功能：
...
- **值对象 (Value Objects)**：当你的领域模型中存在值对象时，这些对象通常没有独立的标识，其相等性基于其属性的值。例如，一个 `Customer` 实体可能有一个 `Address` 对象（包含 `Street`、`City`、`State`、`ZipCode`），这个 `Address` 总是属于某个 `Customer`，不会独立存在或被共享。你可以将 `Address` 建模为 `Customer` 的从属实体。
