...
简单日志记录是 EF Core 提供的一种**快速、开箱即用的日志输出方式**，用于开发者在**调试阶段**方便地查看 EF Core 的行为（如 SQL 执行、跟踪状态、模型验证等）。它使用的是 `.NET 的日志记录系统（ILogger）`，但不需要复杂配置，**只需用一行代码即可启用**。
